...
import { Users, FileText, CheckCircle, Eye, BookOpen } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const pembimbing = JSON.parse(localStorage.getItem('user'));

  useEffect(() => {
    const fetchPembimbingDashboardData = async () => {
      setIsLoading(true);
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${API_URL}/pembimbing`, {
    ...
    fetchPembimbingDashboardData();
  }, []);

  const statCards = [
    { title: 'Mahasiswa Bimbingan', value: stats?.totalMahasiswaBimbingan, icon: Users, color: 'text-blue-600', bg: 'bg-blue-100' },
    { title: 'Jurnal Pending', value: stats?.jurnalPending, icon: FileText, color: 'text-yellow-600', bg: 'bg-yellow-100' },
    { title: 'Jurnal Disetujui', value: stats?.jurnalsApproved, icon: CheckCircle, color: 'text-green-600', bg: 'bg-green-100' }
  ];

  return (
    <div className="space-y-8 p-6 md:p-8">
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-8 text-white shadow-lg">
        <h2 className="text-3xl font-bold mb-2">Selamat Datang, {pembimbing?.nama_lengkap || 'Pembimbing'}!</h2>
        <p className="text-blue-100 text-lg">
          Tinjau dan kelola jurnal kegiatan dari mahasiswa bimbingan Anda.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-md border border-gray-200">
        <div className="p-6 border-b flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500">5 jurnal terbaru yang perlu Anda verifikasi</p>
          </div>
          <button onClick={() => navigate('/pembimbing/jurnal')} className="text-sm font-semibold text-blue-600 hover:underline">
            Lihat Semua
          </button>
        </div>
              ...
              <div key={jurnal.id} className="p-6 hover:bg-gray-50 transition-colors flex justify-between items-center">
                <div>
                  ...
                  <div className="flex items-center space-x-4 text-gray-500 text-sm mt-1">
                    <span className="flex items-center space-x-1"><Users size={16} /><span>{jurnal.nama_lengkap}</span></span>
                    <span className="flex items-center space-x-1"><BookOpen size={16} /><span>{new Date(jurnal.tanggal).toLocaleDateString('id-ID')}</span></span>
                  </div>
                </div>
                <button onClick={() => navigate('/pembimbing/jurnal')} className="flex items-center space-x-1 px-3 py-1.5 text-blue-600 bg-blue-50 rounded-lg text-sm font-medium hover:bg-blue-100">
                  <Eye size={16} />
                  <span>Review</span>
                </button>
              </div>
